...
namespace Wwise.API.Editor.SoundBankDirectoryWatcher.Common
{
	...
	public class AkSoundBankDirectoryWatcher 
	{
		...
		{
			StartWatcher(AkBasePathGetter.GetPlatformName(), AkWwiseInitializationSettings.ActivePlatformSettings.InitialLanguage);
		}
		...
		public void StartWatcher(string inPlatformName, string inLangaugeName)
		{
			UnityEditor.EditorApplication.update += OnEditorUpdate;
			WwiseProjectDatabase.SoundBankDirectoryUpdated += AkPlatformPluginList.ExecuteParse;
			Setup(inPlatformName, inLangaugeName);
		}
		...
		public void Setup(string inPlatformName, string inLangaugeName)
		{
			platformName = inPlatformName;
			languageName = inLangaugeName;
			forceUpdate = true;
		}
		...
		private async Task InitProjectDB(string filename, string wProjPath, DateTime time)
		{
			...
			{
				if (!await WwiseProjectDatabase.InitAsync(filename, platformName, languageName))
				{	
					var userWarning = "";
